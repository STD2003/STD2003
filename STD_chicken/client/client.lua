...
function GeneratePropCoords()
	while true do
		...
		CoordX = Config.zone.chicken.coords.x + modX
		CoordY = Config.zone.chicken.coords.y + modY
...
function ValidateCoord(Coord)
	if spawnedProps > 0 then
		...
		if GetDistanceBetweenCoords(Coord, Config.zone.chicken.coords, false) > 50 then
			validate = false
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.zone.chicken.coords, true) < Config.radarbody then
			SpawnProps()
			Citizen.Wait(500)
		else
			Citizen.Wait(500)
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Props, 1 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(Props[i]), Config.zone.chicken.coords, true) >= Config.radarPanda then 
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				ClearPedTasks(playerPed)									
					TriggerServerEvent('STD_chicken:pickedUp')
					IsPickingUp = false
					canuse = false
				end
			end

...
RegisterServerEvent('STD_Cow:pickedUp')
AddEventHandler('STD_Cow:pickedUp', function()
	...
	if xItem.limit ~= -1 and xItem.count >= xItem.limit then
		...
		if xItem.limit ~= -1 and (xItem.count + xItemCount) > xItem.limit then
			...
			TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'JobCow', sendToDiscord, xPlayer.source, '^2')
		else
		...
		if Config.ItemBonus ~= nil then
			for k,v in pairs(Config.ItemBonus) do
				if math.random(1, 100) <= v.Percent then
					...
					TriggerEvent('azael_dc-serverlogs:sendToDiscord', 'JobCow', sendToDiscord, xPlayer.source, '^9')
				end
			end
		end
	end

...
function GeneratePropCoords()
	while true do
		...
		CoordX = Config.zone.Cow.coords.x + modX
		CoordY = Config.zone.Cow.coords.y + modY
...
function ValidateCoord(Coord)
	if spawnedProps > 0 then
		...
		if GetDistanceBetweenCoords(Coord, Config.zone.Cow.coords, false) > 50 then
			validate = false
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.zone.Cow.coords, true) < Config.radarbody then
			SpawnProps()
			Citizen.Wait(500)
		else
			Citizen.Wait(500)
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Props, 1 do
			...
			if GetDistanceBetweenCoords(GetEntityCoords(Props[i]), Config.zone.Cow.coords, true) >= Config.radarPanda then 
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				ClearPedTasks(playerPed)									
					TriggerServerEvent('STD_Cow:pickedUp')
					IsPickingUp = false
					canuse = false
				end
			end
